...
int main(int argc, char **argv) {
	...
	SDL_GPUTexture* pipeTexture = Loader::load_texture(device, "resources/textures/default.png");
	SDL_GPUTexture* engiTexture = Loader::load_texture(device, "resources/textures/engi.png");
	...
	for (int i = 0; i < pipePairsCount; ++i){
		...
		top->texture = pipeTexture;
		bot->texture = pipeTexture;
	...
	while (running) {
		...
		SDL_GPUTextureSamplerBinding texBind{ nullTexture, textureSampler };
		...
		sceneData.time = SDL_GetTicks()/1000.f;

		SDL_GPUTextureSamplerBinding tex_binds[1];
		tex_binds[0].texture = painis->texture;
		tex_binds[0].sampler = textureSampler;

		SDL_BindGPUFragmentSamplers(renderPass, 0, tex_binds, 1);

		SDL_PushGPUFragmentUniformData(commandBuffer, 0, &sceneData, sizeof(sceneData));
		SDL_PushGPUVertexUniformData(commandBuffer, 0, &finalMPV, sizeof(Mat4));
		SDL_DrawGPUIndexedPrimitives(renderPass, 6, 1, 0, 0, 0);

		SDL_GPUTexture* lastTexture = nullTexture;
		...
		for (auto& s : scene.sprites) {
			if (s->firstIndex == 0) continue;
			if (s->texture != lastTexture) {
				if (s->texture == 0) continue;

				SDL_GPUTextureSamplerBinding tex_binds[1];
				tex_binds[0].texture = s->texture;
				tex_binds[0].sampler = textureSampler;

				SDL_BindGPUFragmentSamplers(renderPass, 0, tex_binds, 1);
				lastTexture = s->texture;
			}
			SDL_DrawGPUIndexedPrimitives(renderPass, 6, 1, s->firstIndex, 0, 0);
		}
	...
	SDL_ReleaseGPUTexture(device, nullTexture);
	SDL_ReleaseGPUTexture(device, pipeTexture);
	SDL_ReleaseGPUTexture(device, engiTexture);
